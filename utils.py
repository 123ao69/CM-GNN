...
def get_rw_adj(data, norm_dim=1, fill_value=0., num_nodes=None, dtype=None):
    edge_index1 = data.edge_index
    num_nodes = maybe_num_nodes(edge_index1, num_nodes)

    edge_weight1 = torch.ones((edge_index1.size(1),), dtype=dtype,
                              device=edge_index1.device)

    data.edge_index, _ = remove_self_loops(data.edge_index)

    if not fill_value == 0:
        edge_index1, tmp_edge_weight = add_remaining_self_loops(
            edge_index1, edge_weight1, fill_value, num_nodes)
        assert tmp_edge_weight is not None
        edge_weight1 = tmp_edge_weight

    row, col = edge_index1[0], edge_index1[1]
    indices = row if norm_dim == 0 else col
    deg = scatter_add(edge_weight1, indices, dim=0, dim_size=num_nodes)
    deg_inv_sqrt = deg.pow_(-1)
    edge_weight1 = deg_inv_sqrt[indices] * edge_weight1 if norm_dim == 0 else edge_weight1 * deg_inv_sqrt[indices]
    data.edge_index = edge_index1
    # edge_weight1[-2 * data.num_classes:] = -edge_weight1[-2 * data.num_classes:]
    data.edge_attr = edge_weight1
    degrees = degree(data.edge_index[0], num_nodes=data.num_nodes)
    data.edge_weight = tmp_edge_weight
    data.edge_weight[-len(degrees):] = data.edge_weight[-len(degrees):]
...
def collapsing_particle(data):
    y_hat = data.y[data.train_mask]
    a = torch.arange(data.num_nodes)[data.train_mask]
    nodes_by_category = [[] for _ in range(data.num_classes)]
    for index, label in zip(a, y_hat):
        nodes_by_category[label].append(index)
    for label, nodes in enumerate(nodes_by_category):
        if nodes:  # 确保列表不为空
            representative_node = nodes[0]
            for node in nodes[1:]:
                new_edge1 = torch.tensor([[node, representative_node], [representative_node, node]], dtype=torch.long)
                data.edge_index = torch.cat((data.edge_index, new_edge1), dim=1)
    first_node = nodes_by_category[0][0]
    for i in range(data.num_classes):
        if i != 1 and data.num_nodes == 183:
            new_edge2 = torch.tensor([[first_node, nodes_by_category[i][0]],
                                      [nodes_by_category[i][0], first_node]], dtype=torch.long)
            data.edge_index = torch.cat((data.edge_index, new_edge2), dim=1)
