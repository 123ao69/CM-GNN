...
def get_dataset(opt: dict, data_dir, use_lcc: bool = False) -> InMemoryDataset:
    ...
    dataset.data.num_classes = int(max(dataset.data.y) + 1)

    if ds == 'texas' or ds == 'wisconsin' or ds == "cornell" or ds == 'film':
        print(dataset.data.train_mask.shape)
        dataset.data.train_mask = dataset.data.train_mask[:, 0]
        dataset.data.val_mask = dataset.data.val_mask[:, 0]
        dataset.data.test_mask = dataset.data.test_mask[:, 0]

    return dataset


def DropAndRecoverEdge(data, sampling_percent):
    nnz = data.edge_index.shape[1]
    perm = np.random.permutation(nnz)
    preserve_nnz = int(nnz * sampling_percent)
    drop_perm = perm[:preserve_nnz]
    remain_perm = perm[preserve_nnz:]

    row = data.edge_index[0][drop_perm]
    col = data.edge_index[1][drop_perm]

    data.edge_index = torch.stack((row, col), dim=0)
    data.edge_attr = data.edge_attr[drop_perm]
