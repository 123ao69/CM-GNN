...
from data import get_dataset, set_train_val_test_split, DropAndRecoverEdge
from best_params import best_params_dict
from utils import ROOT_DIR, collapsing_particle, get_rw_adj
import torch
...
def main(cmd_opt):
    ...
    collapsing_particle(dataset.data)

    get_rw_adj(dataset.data, norm_dim=1, fill_value=opt['self_loop_weight'],
               num_nodes=dataset.data.num_nodes, dtype=dataset.data.x.dtype)

    model = GNNEarly(opt, dataset, device).to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--add_source', dest='add_source', action='store_false',
    ...
    print(f'mean of test acc is {np.mean(test_acc_list)}, std of test acc is {np.std(test_acc_list)}')
