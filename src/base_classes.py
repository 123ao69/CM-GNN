...
class ODEFunc(MessagePassing):
    ...
    def __init__(self, opt, data, device):
        ...
        self.alpha_train = nn.Parameter(torch.tensor(0.0) * torch.ones(1, self.opt['hidden_dim']))
        self.theta_train = nn.Parameter(torch.tensor(0.0))
        self.delta_a = nn.Parameter(torch.tensor(0.0) * torch.ones(1, self.opt['hidden_dim']))
        self.probs = nn.Parameter(torch.full((len(data.edge_weight),), 0.15))
